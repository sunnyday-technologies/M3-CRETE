...
**Test repo:** `[local-path-redacted] (real project, not fixture)
...
.cadclaw_venv\Scripts\python.exe -m pip install -e [local-path-redacted]
...
| `doctor` venv-home probe | Test machine had a leftover venv with `pyvenv.cfg` pointing at a deleted Python (`[home-path-redacted] from another machine). Doctor ran on the fresh venv I built and correctly resolved `home = [home-path-redacted] | The probe needs to keep working when `home` exists; needs an explicit *failing* test where `pyvenv.cfg.home` points at a missing path → fail with the path printed. |
...
Run `examples/init_rules.py` followed by a tuned harness against `[local-path-redacted] (no shim BOM needed once HIGH-1 is fixed) and expect:
...
- This file: `[local-path-redacted]
- Test fixtures (real-world): `[local-path-redacted] `[local-path-redacted] `[local-path-redacted]
- Generated harness output: `[local-path-redacted]
- v0.6 worktree: `[local-path-redacted]
