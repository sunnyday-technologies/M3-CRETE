...
# Lite: parametric boxes matching Components/ STEP dimensions.
plate_20_80 = cq.Workplane("XY").box(127, 3, 88)    # Gantry Plate 20-80mm
vwheel      = cq.Workplane("XY").box(10.2, 23.9, 23.9)  # Solid V Wheel
...
CBEAM_W = 40           # profile width (X)
CBEAM_H = 80           # profile height (Z)
CBEAM_L = 1200         # rail length (Y)
CBEAM_CHAN_W = 20       # channel opening width (Z)
CBEAM_CHAN_D = 20       # channel depth from inner face (X) — half of beam width

# Outer box minus channel cut = C-beam profile, extruded along Y.
# Channel is on the +X face (inboard side for L rail).
_beam_outer = cq.Workplane("XY").box(CBEAM_W, CBEAM_L, CBEAM_H,
                                     centered=(True, False, True))
_chan_cut   = cq.Workplane("XY").box(CBEAM_CHAN_D, CBEAM_L, CBEAM_CHAN_W,
                                     centered=(True, False, True))
_chan_cut   = _chan_cut.translate(((CBEAM_W - CBEAM_CHAN_D) / 2, 0, 0))
e_cbeam     = _beam_outer.cut(_chan_cut)

# Left rail: channel faces inboard (+X). Origin at center X, Y=0, center Z.
add(e_cbeam, "zpY_L", DRK,
    L(ZPY_L_X - 10, D/2 - CBEAM_L/2, ZP))
# Right rail: channel faces inboard (-X). Mirror across YZ.
e_cbeam_r = e_cbeam.mirror("YZ")
add(e_cbeam_r, "zpY_R", DRK,
    L(ZPY_R_X + 10, D/2 - CBEAM_L/2, ZP))
...
idler_sm     = cq.Workplane("XY").box(12.7, 22, 22)               # Smooth Idler (lite)
...
# L-brackets: parametrized as 65x69x69 boxes (lite version).
...
out = os.path.join(os.path.dirname(__file__), "M3-2_Assembly_lite.step")
