...
- **Repo:** `D:\SunnydayTech\M3-CRETE\` → `github.com/sunnyday-technologies/M3-CRETE`
...
- `D:\SunnydayTech\VICTORiA\` — Business operating system (separate git, local only)
- `D:\SunnydayTech\NemoCrete\` — Proprietary pump system (NO git, never publish)
- `D:\SunnydayTech\M3-CRETE\` — This project (public GitHub)
...
- **Venv (laptop, canonical 2026-04-15):** `C:\Users\Sunny\Projects\M3-CRETE\cad_venv\Scripts\python.exe` (Python 3.11.9 + CadQuery 2.7.0, gitignored)
- **Underlying Python:** `C:\Users\Sunny\AppData\Local\Programs\Python\Python311\python.exe`
...
- **Deprecated venv locations:** `Z:/SunnydayTech/M3-CRETE/cad_venv/` (never re-map Z:), `Y:/SunnydayTech/M3-CRETE/cad_venv/` (network, stale), `D:/m3crete_fix/` (old temp clone)
...
## Product Decisions
- **Pump system is OUT OF SCOPE** — separate product called NemoCrete (proprietary, `D:\SunnydayTech\NemoCrete\`)
...
- **Laptop canonical:** `C:\Users\Sunny\Projects\M3-CRETE` (moved from `Y:\SunnydayTech\M3-CRETE` on 2026-04-15 to survive network-drive remount failures across reboots)
- **Publications nested inside** at `Publications/` — gitignored, local only, never published via this repo
- **Desktop canonical:** still `D:\SunnydayTech\M3-CRETE` (per-machine session history, Syncthing pairing pending)
- **Old session transcripts** live under `C:\Users\Sunny\.Codex\projects\` across multiple path-hash folders — grep on demand
